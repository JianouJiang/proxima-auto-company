/**
 * SixDegrees AI Agent Worker
 * Cloudflare Worker that powers the AI agent connection service with email sending
 */
...
/**
 * Send email via MailChannels (Cloudflare Email Routing)
 */
async function handleSendEmail(request, env, corsHeaders) {
  ...
  try {
    ...
    if (campaign_id) {
      const emailId = generateId();
      await env.DB.prepare(`
        INSERT INTO email_outreach (id, campaign_id, recipient_email, recipient_name, subject, body, status, error_message)
        VALUES (?, ?, ?, ?, ?, ?, 'failed', ?)
      `).bind(
        emailId,
        campaign_id || 'test',
...
async function handleDashboard(request, env, corsHeaders) {
  ...
  // Get email status for each campaign
  const campaignsWithEmails = await Promise.all(campaigns.results.map(async (c) => {
    const emails = await env.DB.prepare(
      'SELECT * FROM email_outreach WHERE campaign_id = ? ORDER BY created_at DESC'
    ).bind(c.id).all();
